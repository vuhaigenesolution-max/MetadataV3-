...
def process_excel(source_path, template_path, output_path):
    ...
    if safe_run_name.strip('_') == "":
        wb_source.close()
        raise ValueError("Run name không hợp lệ sau khi làm sạch.")
    ...
    # Kiểm tra file output tồn tại trước khi làm bất cứ điều gì
    target_folder = os.path.join(output_path, barcode_name)
    csv_path = os.path.join(target_folder, 'barcode.csv')
    if os.path.exists(csv_path):
        raise FileExistsError(
            f"File đã tồn tại: {csv_path}. Vui lòng xóa hoặc đổi tên trước khi chạy lại."
        )

    # 2. Đọc PRIMER LOCKED từ template gốc (read-only, không sửa template)
    ...
    if not rows_primer:
        raise ValueError("Sheet 'PRIMER LOCKED' trong template rỗng.")

    max_col_primer = max((len(r) for r in rows_primer if r), default=0)
    rows_primer = [r[:max_col_primer] for r in rows_primer]
    ...
    # 3. Tính toán lookup (thay thế VLOOKUP của Excel)
    result_df = pd.DataFrame({'A': df_T['T']})
    ...
    print(f"Đã tạo file CSV tại: {csv_path}")
    return csv_path
